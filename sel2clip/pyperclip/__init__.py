...
STR_OR_UNICODE = str if PY2 else str # For paste(): Python 3 uses str, Python 2 uses unicode.
...
def _stringifyText(text):
    if PY2:
        acceptedTypes = (str, str, int, float, bool)
    else:
...
def set_clipboard(clipboard):
    ...
    if clipboard not in clipboard_types:
        raise ValueError('Argument must be one of %s' % (', '.join([repr(_) for _ in list(clipboard_types.keys())])))
